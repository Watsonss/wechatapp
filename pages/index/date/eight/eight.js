Page({
  ...
  getareanum: function (e)
  {
    ...
    {
      var str = "area[" + i + "].num";
      this.setData({ [str]: ((this.data.business[i].num + this.data.business[i + 17].num + this.data.business[i + 34].num) / 3.0) })
  ...
  predicrtnum:function(e)
  {
    ...
    if(e.time<11)
    {
      for (var i = 0; i <e.time-6; i++)
    {e.num+=this.data.area[i].num}
    e.num=e.num/(e.time-5)
  }
  else
  {
      for ( i = e.time-9; i <= e.time - 7; i++)
    {  avenum += this.data.area[i].num
       avetime+= this.data.area[i].time
       powtime += (this.data.area[i].time * this.data.area[i].time)
       timenum += (this.data.area[i].num * this.data.area[i].time) }
    avenum = avenum / 3
    avetime= avetime /3
    b =(timenum - 3 * avenum * avetime) / (powtime - 3 * (avetime * avetime))
    a= avenum-avetime*b
    e.num = a+b*avetime
     }
  }
